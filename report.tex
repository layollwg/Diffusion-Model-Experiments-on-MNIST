...
\subsection*{A.1 Images already in the repository}

\texttt{results/comparison\_chart.png} is committed directly in the repository
(uploaded to \texttt{main} and included in this branch).
No manual download is required for that figure.

\subsection*{A.2 Images that must be downloaded manually}

Place the following image files in a \texttt{figures/} subdirectory next to
\texttt{report.tex} before compiling (right-click each link $\to$ Save As):
...
\subsection*{A.3 Compilation}

Compile with \texttt{pdflatex} (two passes for the table of contents):

\begin{verbatim}
mkdir figures
# download the 7 images above into figures/ using the names in the table
pdflatex report.tex
